...
import { stocks as initialStocks } from '../../constant'; 
import { mutuals as initialMutuals } from '../../constant'; 
import { ipos as initialIpos} from '../../constant'; 
import { WatchlistData as initialWatchlist } from '../../constant'; 
import { CgBookmark, CgMathPlus } from "react-icons/cg";
import { url } from '../url';
import Hamburgernav from '../components/Hamburgernav';
...
const UponLogin = () => {
  const [stocks] = useState(initialStocks);
  const [mutuals] = useState(initialMutuals);
  const [ipos] = useState(initialIpos);
  const [WatchlistData] = useState(initialWatchlist);
  const [isOpen, setIsOpen] = useState(false);
  const [isVisible, setIsVisible] = useState(false);
  const [portfolioValue,setPortfoliovalue] = useState(0);
  ...
  return (
    ...
    <div className="grid grid-cols-3 gap-4 mt-4 text-white divide-x">
      <div>
        ...
        <br />
        {stocks.map((stock, index) => (
            <div className={`bg-[#0f172a] p-4 rounded  mt-6 ml-8 mr-6 hover:border border-white transition duration-300 min-h-[185px] relative ${index % 2 === 0 }`} key={stock.stockname} onClick={()=>openStockInfo(stock)} >
              <div className="flex justify-between items-center text-white">
                <div>
                  <p className="font-semibold">{stock.stockname}</p>
                  <br/>
                  <p>Mkt Price: {stock.currentPrice}</p>
                ...
                <div>
                  ...
                  <div className="flex items-center">
                    <p>% P/L:</p>
                    <p className={stock.unrealizedPL >= 0 ? "text-green-500" : "text-red-500"}>{stock.unrealizedPL}%</p>
                  </div>
                </div>
              </div>
              <div className="absolute top-2 right-2 hover:scale-110">
      ...
      <div className='mb-8'>
            ...
            <div className={`bg-[#0f172a] p-4 rounded  mt-6 ml-8 mr-6 hover:border border-white transition duration-300 min-h-[185px] ${index % 2 === 0 }`} key={mutual.mutualname} onClick={()=>openStockInfo(mutual)} >
             <div className="flex justify-between items-center text-white">
                <div>
                  <p className="font-semibold">{mutual.mutualname}</p>
                  <br/> 
                  <p>Current Price: {mutual.minprice}</p>
                ...
                <div>
                  ...
                  <div className="flex items-center">
                    <p>1Y Returns:</p>
                    <p className="text-green-500 ml-2">{mutual.oneYreturns}%</p>
                  </div>
                </div>
              </div>
        ...
        <br />
        {ipos.map((ipo, index) => (
            <div className={`bg-[#0f172a] p-4 rounded  mt-6 ml-8 mr-6 hover:border border-white transition duration-300 min-h-[185px] ${index % 2 === 0 }`} key={ipo.ipoName} onClick={()=>openStockInfo(ipo)} >
              <div className="flex justify-between items-center text-white">
                <div>
                  <p className="font-semibold">{ipo.mutualname}</p>
                  
                  <p>Issue Price: {ipo.minprice}</p>
                  <p>Issue Date: {ipo.issueDate}</p>

                  
                </div>  
                <div>
                  <br></br><br />
                  {/* <p>Max Retail: {ipo.maxRetailValue}</p> */}
